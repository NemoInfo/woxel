...
impl MaskUniform {
    pub fn bind(&self, device: &Device) -> ([Buffer; 5], [Vec<u8>; 5], BindGroup, BindGroupLayout) {
    ...
    fn create_buffers(&self, device: &Device, buffer_contents: &[Vec<u8>; 5]) -> [Buffer; 5] {
        buffer_contents
            .iter()
            .enumerate()
            .map(|(idx, contents)| {
